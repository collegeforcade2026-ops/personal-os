-- Enable vector extension (must be done in Supabase dashboard first)
...
create table daily_logs (
  ...
  notes       text,   -- stores JSON for habits/nutrition/finance/goals
  mood        text,
  created_at  timestamptz default now(),
  updated_at  timestamptz default now(),
...
-- ─── RLS: deny-all on all tables (service role bypasses RLS) ────────
...
-- Deny-all policies (no authenticated user access — service role only)
